    ...
    private void ResultActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ResultActionPerformed
        if (jTable1.getSelectedRow() > -1){
            ...
            { 
                ...
                ListThread.add(new MyThread(down,up,StepD));
                down = up;
                ListThread.getLast().start();
                System.out.println("Thread start: "+ ListThread.getLast().getName());
            }
            for(MyThread thread: ListThread){
                try{
                    thread.join();
                    Square += thread.getResult();
                    System.out.println("Thread end: "+ thread.getName() +"  Result: "+ thread.getResult());
